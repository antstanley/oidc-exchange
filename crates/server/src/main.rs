...
use oidc_exchange::middleware::error_handler::panic_handler;
use oidc_exchange::middleware::request_id::request_id_layer;
use oidc_exchange::routes;
...
fn load_config() -> Result<AppConfig, Box<dyn std::error::Error>> {
    ...
    // When env == "default" the env_config and default_config are the same file;
    // the extra read is harmless.
    ...
    match config.key_manager.adapter.as_str() {
        "local" => {
            let local_cfg = config.key_manager.local.as_ref().ok_or_else(|| {
                Error::ConfigError {
                    detail: "key_manager.adapter is 'local' but [key_manager.local] section is missing".into(),
                }
            })?;
        ...
        "kms" => {
            let kms_cfg = config.key_manager.kms.as_ref().ok_or_else(|| {
                Error::ConfigError {
                    detail: "key_manager.adapter is 'kms' but [key_manager.kms] section is missing".into(),
                }
            })?;

            // Build KMS client synchronously using a blocking load.
            // The AWS config is loaded in the tokio runtime context of main().
    ...
    match config.audit.adapter.as_str() {
        ...
        "cloudtrail" => {
            let ct_cfg = config.audit.cloudtrail.as_ref().ok_or_else(|| {
                Error::ConfigError {
                    detail: "audit.adapter is 'cloudtrail' but [audit.cloudtrail] section is missing".into(),
                }
            })?;
    ...
    match config.user_sync.adapter.as_deref() {
        Some("webhook") => {
            let wh_cfg = config.user_sync.webhook.as_ref().ok_or_else(|| {
                Error::ConfigError {
                    detail: "user_sync.adapter is 'webhook' but [user_sync.webhook] section is missing".into(),
                }
            })?;
            ...
            Ok(Box::new(oidc_exchange_adapters::webhook::WebhookUserSync::new(
                wh_cfg.url.clone(),
                wh_cfg.secret.clone(),
