...
function cookies(initial?: string) {
  ...
  return {
    get: vi.fn(() => initial),
    set: vi.fn((name: string, value: string, options: Record<string, unknown>) => setCalls.push({ name, value, options })),
    delete: vi.fn((name: string, options: Record<string, unknown>) => deleteCalls.push({ name, options })),
    setCalls,
    deleteCalls,
  };
}

async function captureRedirect(call: () => PromiseLike<unknown> | unknown): Promise<{ status: number; location: string }> {
  try {
    await call();
  } catch (error) {
...
describe("session hook", () => {
  it("allows a protected request only with verified admin claims in locals", async () => {
    ...
    expect((event as { locals: Record<string, unknown> }).locals).toEqual({ userId: "operator-1", claims });
    expect(jar.deleteCalls).toHaveLength(0);
  });

  it.each([undefined, "invalid-token"])("redirects invalid session %j and clears a present cookie", async (token) => {
    if (token) verifyAccessToken.mockRejectedValue(new Error("invalid"));
    const { handle } = await import("./hooks.server");
    const jar = cookies(token);
    const event = { url: new URL("https://console.example/users"), cookies: jar, locals: {} } as never;
    ...
    if (token) expect(jar.deleteCalls[0]).toMatchObject({ name: "__Host-admin_session", options: { path: "/", secure: true, httpOnly: true, sameSite: "strict" } });
  });
  ...
  it("redirects verified non-admin sessions to denied without clearing", async () => {
    ...
    const event = { url: new URL("https://console.example/"), cookies: jar, locals: {} } as never;
    expect(await captureRedirect(() => handle({ event, resolve: vi.fn() } as never))).toMatchObject({ location: "/denied" });
    expect(jar.deleteCalls).toHaveLength(0);
  });
});
...
describe("login and logout", () => {
  it("sets the exact hardened cookie from verified expiry", async () => {
    ...
    const jar = cookies();
    const request = new Request("https://console.example/login", { method: "POST", body: new URLSearchParams({ token: "signed-token" }) });
    const result = await captureRedirect(() => actions.default({ request, cookies: jar } as never) as Promise<unknown>);
    expect(result).toMatchObject({ status: 303, location: "/" });
    expect(jar.setCalls).toHaveLength(1);
    expect(jar.setCalls[0]).toMatchObject({ name: "__Host-admin_session", value: "signed-token", options: { path: "/", secure: true, httpOnly: true, sameSite: "strict" } });
  ...
  it("returns 401 and never persists verification failures", async () => {
    ...
    const { actions } = await import("./routes/login/+page.server");
    const jar = cookies();
    const request = new Request("https://console.example/login", { method: "POST", body: new URLSearchParams({ token: "invalid-token" }) });
  ...
  it("login load verifies existing cookies and clears invalid ones", async () => {
    ...
    expect(jar.deleteCalls[0]).toMatchObject({ name: "__Host-admin_session", options: { path: "/", secure: true, httpOnly: true, sameSite: "strict" } });
  });
  ...
  it("verified non-admin login follows denied flow without a cookie", async () => {
    ...
    const { actions } = await import("./routes/login/+page.server");
    const jar = cookies();
    const request = new Request("https://console.example/login", { method: "POST", body: new URLSearchParams({ token: "signed-user-token" }) });
    expect(await captureRedirect(() => actions.default({ request, cookies: jar } as never) as Promise<unknown>)).toMatchObject({ location: "/denied" });
    expect(jar.setCalls).toHaveLength(0);
  });
  ...
  it("logout deletes with matching host-cookie scope and attributes", async () => {
    ...
    const jar = cookies("signed-token");
    expect(await captureRedirect(() => load({ cookies: jar } as never) as Promise<unknown>)).toMatchObject({ location: "/login" });
    expect(jar.deleteCalls[0]).toMatchObject({ name: "__Host-admin_session", options: { path: "/", secure: true, httpOnly: true, sameSite: "strict" } });
