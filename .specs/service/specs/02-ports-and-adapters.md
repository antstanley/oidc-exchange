...
|---|---|---|---|
| UserRepository + SessionRepository | DynamoDB | `adapters/dynamo` | single-table, GSI1; sessions, retirement items, and a transactionally maintained per-user roster; see [08-persistence.md](08-persistence.md) |
| UserRepository + SessionRepository | Postgres | `adapters/postgres` | `users` + `sessions` + `retired_refresh_tokens` tables, JSONB columns, `sqlx` |
| UserRepository + SessionRepository | SQLite | `adapters/sqlite` | JSON-as-TEXT, WAL mode, `sqlx`; same three tables |
| SessionRepository | LMDB | `adapters/lmdb` | embedded; `heed`; four DBs — `sessions`, `user_sessions`, `retired_tokens`, `family_index`; batched cleanup |
| SessionRepository | Valkey/Redis | `adapters/valkey` | `fred`; `{prefix}session:{hash}`, `{prefix}retired:{hash}`, `{prefix}family:{family_id}` set, `{prefix}user_sessions:{user_id}` set (TTL bumped via `EXPIRE … GT`), `{prefix}active_sessions` counter; Lua rotation and pipelined writes; cleanup prunes index sets and reconciles the counter |
