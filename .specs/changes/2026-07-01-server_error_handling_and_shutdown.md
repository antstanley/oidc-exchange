...
| ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`.specs/service/specs/04-http-api.md`](../service/specs/04-http-api.md)                       | Modify the `/revoke` route row and the _Always-200 revoke_ Decision; extend the request-id middleware entry with the per-request span; add graceful shutdown and the timeout layer to Bootstrap. The error-mapping "log the internal detail" sentence already describes the target state and needs only a request-id mention |
| [`.specs/service/specs/06-configuration.md`](../service/specs/06-configuration.md)             | `[server]` section: add the `request_timeout` key                                                                                                                                                                                                                                                                            |
...
>    deadline, after which stragglers are aborted and the process exits. A request-timeout
>    layer on the router bounds slow clients at `server.request_timeout` (default 30 s).
...
4. `crates/server/src/main.rs:36-37`: `axum::serve(...).with_graceful_shutdown(...)` awaiting
   ...
   driven by a new `request_timeout: String` field on `ServerConfig`
