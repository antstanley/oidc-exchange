...
**Status:** Done · **Layout:** kanban · **Date:** 2026-08-05 · **Owner:** Ant Stanley · **Source spec:** [.specs/changes/merged/2026-08-05-audit_and_throttle_authentication_failures.md](../../changes/merged/2026-08-05-audit_and_throttle_authentication_failures.md)
...
All seven packages have moved from `backlog/` to `done/`. Tasks 01–07 independently passed their correctness and definition-of-done checks, including their scoped acceptance evidence. The dependency graph is fully satisfied: 01 enabled 02–05; 03–05 enabled 06; and 06 enabled 07. Final verification passed: `cargo test --workspace` and `cargo nextest run --workspace` — **422 passed, 27 skipped** for nextest.
...
| 06 · public audit and throttle end-to-end | 03, 04, 05 | review | router-level tests demonstrate mandatory recording, provenance, limits, retry headers, no outbound work after denial, and passing full-workspace verification |
...
| M3 — public boundary enforcement | 05, 06 | a public caller reaches `/token` through trusted address resolution, throttle, audit, access logging, and 429 behavior without excess provider work | server E2E tests pass; final `cargo test --workspace` and `cargo nextest run --workspace` pass, with no `providers.*.adapter` baseline failures remaining |
...
- Historical planning recorded three `providers.*.adapter` configuration-test failures as baseline noise. Final execution evidence supersedes that baseline: both full-workspace test commands pass and no such failures remain; this PR does not attribute their resolution to its own changes.
