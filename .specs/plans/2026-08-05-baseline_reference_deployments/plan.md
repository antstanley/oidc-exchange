...
- **Already built.** The current branch provides the deployable examples, CI workflow, adapter migrations, Lambda bootstrap/config placeholder resolution, and demo application. The code read confirmed the defects remain: `fred` lacks a TLS feature (`crates/adapters/Cargo.toml:31`), SQLite creates its file through `sqlx` (`crates/adapters/src/sqlite/mod.rs:65-86`), the old Postgres example index remains (`examples/linux-postgres/init.sql:14`), and the demo loader only decodes the JWT (`examples/aws-web/demo-app/src/routes/authenticated/+page.server.ts:5-35`). These are implementation targets, not completed work.
...
- **Sibling dependencies, not scope.** The plan consumes `oidc-exchange config check` from `2026-08-05-resolve_config_placeholders_all_channels.md`; consumes the durable-audit outcome from `2026-08-05-audit_and_throttle_authentication_failures.md`; coordinates the AWS KMS algorithm fix with `2026-08-05-fail_closed_across_config_and_adapters.md`; and leaves release provenance, installer, advisory, and `.gitignore` work to `2026-08-05-harden_release_supply_chain.md`. These sibling files are not present in this workspace, so their cited paths remain external dependencies and no sibling implementation is planned here.
