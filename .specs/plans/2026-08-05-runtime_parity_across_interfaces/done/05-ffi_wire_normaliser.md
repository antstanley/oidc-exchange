...
- F3 panic-boundary regressions use a private `cfg(test)` router constructor with injected Tower services: one panics while its request future is first polled, one returns a body that panics from `poll_frame`. Both are wrapped by an external `catch_unwind`, produce an exact empty generic 500, retain only a valid bounded request ID, suppress invalid IDs and fixture secrets from response/captured tracing, and leave the runtime reusable. The deprecated synchronous `block_on` shim is exercised over the same injected boundary and does not unwind.
