...
**Post-review note:** the Postgres corpus seed in `session_span_leak_corpus.rs` is now get-or-created via `UserRepository::get_user_by_external_id`, making `postgres_session_spans_never_render_sentinels` idempotent across reruns against persistent state.
