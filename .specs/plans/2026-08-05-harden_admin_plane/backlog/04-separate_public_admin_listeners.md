...
- [ ] No public router contains internal routes; `all` binds two distinct listeners, while `exchange` and `admin` bind only their plane.
- [ ] Configuration rejects colliding host/port combinations and invalid listener setup before bind; defaults make the admin listener loopback-only.
- [ ] Native shutdown joins both listeners; Lambda/FFI reject or otherwise enforce the documented single-plane constraint without silently serving a merged surface.
- [ ] E2E tests prove `/token` is absent from the admin listener and `/internal/*` is absent from the public listener, with positive health checks for each bound role.
- [ ] Rust format/clippy/affected nextest tests pass; unrelated failures are recorded but not fixed.
- [ ] Reviewable: deployment network policy can expose the admin plane without exposing exchange endpoints.
